...
class plugdetails:
	...
	# Init
	#
	def __init__(self, factory, filter = None):
		self.logger = logging.getLogger ("Plugin.plugdetails")
		...
		self.getDetailsByPluginId (self.factory.plugin.pluginId) # Forces "Indigo" index
	...
	def _resolveObject (self, obj):
		try:
			...
			if type(obj) is indigo.Variable:
				...
				# It's a device
				if obj.pluginId != "":
					plugInfo = self.getPluginCacheItem (obj.pluginId)
	...
	def _resolveIndigoDevice (self, obj):
		try:
			...
			plugInfo = self.getPluginCacheItem ("Indigo")
	...
	#
	# Retrieve an item from cache or cache it if its not there
	#
	def getPluginCacheItem (self, pluginId):
		try:
			#indigo.server.log(pluginId)
				
			if pluginId in self.pluginCache:
				plugInfo = self.pluginCache[pluginId]
				
				if "xml" not in plugInfo:
					# Retrieve it
				...
				# This should never get called since we cache basics when the library is loaded
				#indigo.server.log(unicode(self.pluginCache))
	...
	#
	def cachePluginInformation (self):
		try:
	...
	#
	# Get plugin info based on an object
	#
	def getDetailsByObject (self, obj):
		try:
			# Determine if we are reading a plugin device or Indigo device
			objtype = str(type(obj)).replace("<class '", "").replace("'>", "")
			
			#indigo.server.log(objtype)
			
			# Filter out objects we do not yet support
			if objtype == "indigo.ActionGroup":
				self.logger.error ("Plugdetail beta doesn't currently process Action Groups, cannot get actions for '{0}'".format(obj.name))
				return None
			
			elif objtype == "indigo.ControlPage":
				self.logger.error ("Plugdetail beta doesn't currently process Control Pages, cannot get actions for '{0}'".format(obj.name))
				return None
				
			elif "Trigger" in objtype:
				self.logger.error ("Plugdetail beta doesn't currently process Triggers, cannot get actions for '{0}'".format(obj.name))
				return None	
				
			elif objtype == "indigo.Schedule":
				self.logger.error ("Plugdetail beta doesn't currently process Schedules, cannot get actions for '{0}'".format(obj.name))
				return None	
				
			elif objtype == "indigo.Variable":
				self.logger.error ("Plugdetail beta doesn't currently process Variables, cannot get actions for '{0}'".format(obj.name))
				return None		
				
			return self.getDetailsByPluginId (obj.pluginId, obj.deviceTypeId)
	...
	#
	# Get the plugin details for a given object
	#
	def getDetailsByPluginId (self, pluginId, deviceTypeId = ""):
		try:
			# In case this isn't a plugin, assume the Indigo information
			plugInfo = indigo.Dict()
			plugInfo["id"] = "Indigo"
			plugInfo["name"] = "Indigo"
			plugInfo["path"] = ""
			
			# If it's a plugin then get the basic plugin info, otherwise claim it as Indigo
			if pluginId != "":
				if pluginId in self.pluginCache: plugInfo = self.pluginCache[pluginId]
			
			try:
				pluginXML = indigo.Dict()
				
				# If we are caching our own information then also cache the Indigo built-in actions since it's in our plugin folders.  This will
				# actually cache rather than simply return details
				if plugInfo["id"] == self.factory.plugin.pluginId:
					plugInfoEx = indigo.Dict()
					plugInfoEx["id"] = "Indigo"
					plugInfoEx["name"] = "Indigo"
					plugInfoEx["path"] = ""
				
					if os.path.isfile(plugInfo["path"] + "/Contents/Server Plugin/lib/actionslib.xml"):
						pluginXML["actions"] = self._parseActionsXML(plugInfo["path"] + "/Contents/Server Plugin/lib/actionslib.xml")
				...
				if os.path.isfile(plugInfo["path"] + "/Contents/Server Plugin/Devices.xml"):
					pluginXML["devices"] = self._parseDevicesXML(plugInfo["path"] + "/Contents/Server Plugin/Devices.xml")
										
				if os.path.isfile(plugInfo["path"] + "/Contents/Server Plugin/Actions.xml"):
					pluginXML["actions"] = self._parseActionsXML(plugInfo["path"] + "/Contents/Server Plugin/Actions.xml")
				
				plugInfo["xml"] = pluginXML
				
			except Exception as e:
				self.logger.error ("Exception encountered with " + unicode(plugInfo["path"]) + " (this error is NOT critical)")
				self.logger.error (ext.getException(e))
