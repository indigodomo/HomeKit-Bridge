  ...
  if (sessionID) {
    ...
    if (requestType == "start") {
      ...
      if (sessionInfo) {
        ...
        var LANWAN = this.LANWAN || 'None'; // set default Value to None as string
        var ffmpegCommand = this.ffmpegSource;

        let videoInfo = request["video"];

        if (this.debug){
            console.log('VideoInfo:'+JSON.stringify(videoInfo, null,4));
        }
        ...
        if (videoInfo) {
          ...
          if (expectedFPS < fps) {
           ...
           // Pick at Bitrate here to go from low/high
           // Only if LANWAN result found - other ignore and pass unchanged.
           if (LANWAN !='None') {
                if (vbitrate < 700) {
                    //Remove the stream=0, 0
                    ffmpegCommand = ffmpegCommand.substring(0,ffmpegCommand.length-1);
                    ffmpegCommand += LANWAN.charAt(1);
                    console.log('ffmpegCommand WAN equals:'+ffmpegCommand);//get second number == WAN
                } else {
                    ffmpegCommand = ffmpegCommand.substring(0,ffmpegCommand.length-1);
                    ffmpegCommand += LANWAN.charAt(0);  // first number == LAN
                    console.log('ffmpegCommand LAN equals:'+ffmpegCommand);
                }
            }
        }
        ...
        if (this.debug){
            console.log('AudioInfo:'+JSON.stringify(audioInfo, null,4));
        }
        ...
        if(this.audio){
          ffmpegCommand+= ' -map 0:1?' +
            ...
            // Fix the Sample Rate and Buffer/Bufsize
            ' -ar 16k' + //asamplerate + 'k' +
            ' -b:a 8k' + // abitrate + 'k' +
            ' -bufsize 48k' + //abitrate/3 + 'k' +
             ' -ac 1' +
            ' -payload_type 110' +
            ' -ssrc ' + audioSsrc +
            ' -f rtp' +
            ...
            '&pkt_size=188' //Fix audio buffer size to small+ packetsize;
        }
