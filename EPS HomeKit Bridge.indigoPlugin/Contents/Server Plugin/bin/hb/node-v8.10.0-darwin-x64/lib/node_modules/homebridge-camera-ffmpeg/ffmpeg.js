  ...
  if (sessionID) {
    ...
    if (requestType == "start") {
      ...
      if (sessionInfo) {
        ...
        // remove the temp.h264 -- presuming this is what is passed
          // then add temp.ts to get audio stream only
        let audioffmpeg = this.ffmpegSource.substring(0,this.ffmpegSource.length -9) //remove the whole filename temp.h264


        let ffmpegCommand =
          // sets the input framerate expected - needed for a elementary stream
          '-thread_queue_size 512' +
          ' -r '+fps + // sets input to expected
          ' -probesize 32 '+ //limits probesize to minimum is not going to work with elementary stream
          // the source command sent from camera device
          this.ffmpegSource  +
          ' -probesize 32' +
          ' '+ audioffmpeg+'temp.ts'+
          ' -map 0:0' +
          ' -vcodec copy' +//  vcodec +  // copy the codec/stream
          ' -pix_fmt yuv420p' +
       //   ' -r ' + fps +
          ' -loglevel 32' +  // high level logging
   //       ' -reset_timestamps 1' +  // not needed
          ' -f null' + //rawvideo -preset ultrafast -profile:v main -level 1' + //'-tune zerolatency' +
