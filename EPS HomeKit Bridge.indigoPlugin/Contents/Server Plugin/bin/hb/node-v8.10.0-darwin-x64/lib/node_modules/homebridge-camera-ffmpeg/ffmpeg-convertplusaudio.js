...
function FFMPEG(hap, cameraConfig, log) {
  ...
  this.additionalCommandline = ffmpegOpt.additionalCommandline || '';  //change additional to nil
...
FFMPEG.prototype.handleSnapshotRequest = function(request, callback) {
  ...
  // get ride of size downsizing

 // let ffmpeg = spawn('ffmpeg', (imageSource + ' -t 1 -s '+ resolution + ' -f image2 -').split(' '), {env: process.env});

  let ffmpeg = spawn('ffmpeg', (imageSource + ' -t 1 -f image2 -').split(' '), {env: process.env});

  var imageBuffer = Buffer(0);
  this.log("Snapshot from " + this.name + " at " + resolution);
  if(this.debug) console.log('ffmpeg '+imageSource + ' -t 1 -f image2 -');
...
FFMPEG.prototype.handleStreamRequest = function(request) {
  ...
  if (sessionID) {
    ...
    if (requestType == "start") {
      ...
      if (sessionInfo) {
        ...
        // display the stream info Homebrige has got
        //let strsessionInfo = JSON.stringify(sessionInfo,null,4)
        let strvideoinfo = JSON.stringify(videoInfo, null,4)
        //const util = require('util')
        //let strthis =util.inspect(this)
        console.log('--VIDEOINFO:'+strvideoinfo)
        //console.log('--SessionINFO:'+strsessionInfo)
        //console.log('--This Info:'+strthis)

        if (videoInfo) {
          width = videoInfo["width"];
          height = videoInfo["height"];

          let expectedFPS = Math.ceil(videoInfo['fps']);
          if (expectedFPS < fps) {
            fps = expectedFPS;
          }
        ...
        // add audio info to debug stream
        //let straudioInfo = JSON.stringify(audioInfo,null,4)
        //console.log('--AUDIOINFO:'+straudioInfo)
        ...
        if (this.debug){
          ...
          // Relevant for BlueIris Audio if no Audio Option not used
          // using the single raw BlueIris audio stream from one camera
          // then use audio directory instead

        let audioffmpeg = ' ';
        if (this.audio) {
            audioffmpeg = ' '+this.ffmpegSource.replace('video', 'audio')+' ';
            // makes not difference audioffmpeg = audioffmpeg.replace('stream=0','audio=1');
        }

        let ffmpegCommand =
            // sets the input framerate expected - needed for a elementary stream
            //'-r '+ fps +
            // sets input fps to expected
            // '-probesize 32 '+
            //limits probesize to minimum is not going to work with elementary stream and if not here hangs as doesn't repond it accepted time period
            //       '-thread_queue_size 512' +
           // the source command sent from camera device/
            this.ffmpegSource  +
            //       ' -probesize 32' +
            //       ' -thread_queue_size 32' +
            // add the audio input file - ts
                       ''+ audioffmpeg +
            // Map the Video input to output
        //  ' -map_metadata -1' +
            '-r '+fps+
            //  ' -muxdelay 0' +
          ' -map 0:0' +
  ...
  //        '' + additionalCommandline +
          //   ' -r ' + fps +
          // display info level logging from ffmpeg if debug option on
            loglevel +  // high level logging
          //       ' -reset_timestamps 1' +  // not needed
       //  ' -f rawvideo' + //rawvideo -preset ultrafast -profile:v main -level 1' + //'-tune zerolatency' +
         // ' -vf scale=' + width + ':' + height +  //  ' -c copy' +
         //  ' -framerate '+fps+   // set the output framerate
         // ' -b:v ' + vbitrate + 'k' +
         //  ' -bufsize ' +vbitrate +'k' +
         // ' -maxrate ' + vbitrate +'k'+
         //   ' -s:v '+width+':'+height +
          ' -payload_type 99' +' -timestamp now'+
          ' -ssrc ' + videoSsrc +
          ' -f rtp' +
          ...
          '&pkt_size=' + packetsize;
// add the 2nd source the temp.m file that contains the audio
          if(this.audio){
             ffmpegCommand+= ' -map 1:a?' +
            ' -acodec ' + acodec +
            ' -profile:a aac_eld' +
            ' -flags +global_header' +
          //  ' -f null' +
